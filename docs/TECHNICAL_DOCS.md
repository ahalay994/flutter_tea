...
- `getTea()` (строки 623-650) - получение одного чая с поддержкой мульти-тенантности (deviceId) через эндпоинт /device-tea
- `createTeaWithResponse()` (строки 653-665) - создание чая
- `updateTea()` (строки 668-690) - обновление чая с поддержкой мульти-тенантности (deviceId) через эндпоинт /device-tea
- `deleteTea()` (строки 693-710) - удаление чая с поддержкой мульти-тенантности (deviceId) через эндпоинт /device-tea
...
**Основные методы:**
- `getTeas()` (строки 65-72) - получение всех чаёв с поддержкой мульти-тенантности (deviceId) через эндпоинт /device-tea
...
- `getTea()` (строки 272-286) - получение чая с поддержкой мульти-тенантности (deviceId) через эндпоинт /device-tea
